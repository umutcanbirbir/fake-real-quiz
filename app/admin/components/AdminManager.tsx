...
export default function AdminManager({ initial }: { initial: QuizQuestion[] }) {
  ...
  <input type="file" accept="image/jpeg,image/png,image/webp" onChange={(e)=>uploadImage(e.target.files?.[0])} className="rounded bg-zinc-900 p-2"/>
  <input value={form.image_url ?? ""} onChange={(e)=>setForm({...form,image_url:e.target.value})} placeholder="Image URL (optional for all types)" className="rounded bg-zinc-900 p-2"/>
