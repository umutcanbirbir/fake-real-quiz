...
function getContentTypeLabel(questionType: QuestionType) {
  if (questionType === "image") return "IMAGE CHECK";
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-4 rounded border border-zinc-800 p-4 lg:grid-cols-2">
        ...
        <section className="w-full rounded-3xl border border-violet-400/20 bg-zinc-900/80 p-4 shadow-2xl shadow-black/40 backdrop-blur sm:p-6">
          ...
          <div className="mt-4 rounded-2xl border border-zinc-700/70 bg-zinc-950/40 p-4 sm:p-5">
            <div className="mb-4 flex flex-wrap items-center gap-2">
              <span className="rounded-full border border-violet-300/60 bg-violet-400/20 px-3 py-1 text-xs font-bold uppercase tracking-wide text-violet-200">{previewQuestion.category || "General"}</span>
            ...
            {previewQuestion.question_type === "news" ? (
              <>
                {previewQuestion.image_url ? <Image src={previewQuestion.image_url} alt={previewQuestion.headline || "News preview image"} width={1200} height={700} className="mb-4 h-auto w-full rounded-2xl border border-zinc-700/80 object-cover shadow-lg shadow-black/35" unoptimized /> : <p className="mb-4 rounded-xl border border-dashed border-zinc-700 p-4 text-sm text-zinc-400">Optional news image will appear here.</p>}
                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-zinc-500">{previewQuestion.source_name || "Source name preview"}</p>
                <h3 className="mt-2 text-lg font-bold leading-snug text-zinc-100">{previewQuestion.headline || "Headline preview appears here."}</h3>
                <p className="mt-3 text-zinc-300">{previewQuestion.excerpt || "Excerpt preview appears here while you type."}</p>
              </>
            ) : previewQuestion.question_type === "image" ? (
              <>
                {previewQuestion.image_url ? <Image src={previewQuestion.image_url} alt="Image question preview" width={1200} height={800} className="mb-4 h-auto w-full rounded-2xl border border-zinc-700/80 object-cover shadow-lg shadow-black/35" unoptimized /> : <p className="mb-4 rounded-xl border border-dashed border-zinc-700 p-4 text-sm text-zinc-400">Upload or paste an image URL to preview the main visual.</p>}
                <p className="text-lg leading-relaxed text-zinc-100">{previewQuestion.content || "Question text preview appears here."}</p>
              </>
            ) : (
              <>
                ...
                <p className="text-lg leading-relaxed text-zinc-100">{previewQuestion.content || "Your text question preview will appear here."}</p>
              </>
            )}
          </div>
        </section>
